...
#define DFL_FILE_TO_SEND        "" /* modified from original (Atsushi KANDA) */
...
/* modified from original (Atsushi KANDA) */
#define USAGE5 \
    "    file_to_send=%%s    The file path to send a contents to the server\n" \
    "                         default: \"\" (do nothing)\n"                    \
    "                         option: a file path\n"
...
struct options {
    ...
    const char *file_to_send;    /* the file to send to the server / modified from original (Atsushi KANDA) */
} opt;
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) { /* modified from original (Atsushi KANDA) */
usage:
        if (ret == 0) {
            ret = 1;
        }
    ...
    opt.file_to_send        = DFL_FILE_TO_SEND;     /* modified from original (Atsushi KANDA) */
    ...
    for (i = 1; i < argc; i++) {
        ...
        *q++ = '\0';
        ...
        }  else if (strcmp(p, "file_to_send") == 0) {     /* modified from original (Atsushi KANDA) */
            opt.file_to_send = q;
        } else {
            goto usage;
        }
    ...
    /*
     * 6. Read standard input & write
     *    modified from original (Atsushi KANDA)
     */
    if (0 < strlen(opt.file_to_send)) {
        FILE *file;

        mbedtls_printf("  . Reading file content to send ...");

        if ((file = fopen(opt.file_to_send, "r")) == NULL) {
            mbedtls_printf("failed\n  ! Cannot open '%s' for reading.\n",
                            opt.file_to_send);
            goto exit;
        }

        len = (int)fread(buf, sizeof(char), MAX_REQUEST_SIZE, file);
        if (len == 0) {
    ...
    goto close_notify;     /* modified from original (Atsushi KANDA) */
send_request:
    mbedtls_printf("  > Write to server:");
    fflush(stdout);
